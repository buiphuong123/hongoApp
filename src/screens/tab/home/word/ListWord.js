...
export default ListWord = ({navigation, lession, datass}) => {
  ...
  const setMemerize = (userId, wordId) => {
    ...
    axios
      .post(
        'https://nameless-spire-67072.herokuapp.com/language/createMemWord',
        {
          userId: userId,
          wordId: wordId,
        },
  ...
  const setLike = (userId, wordId) => {
    console.log('setlike', data.slice(20 * lession - 20, 20 * lession).length);
    let objIndex = data.findIndex(e => e._id === wordId);
    ...
    axios
      .post(
        'https://nameless-spire-67072.herokuapp.com/language/createLikeWord',
        {
          userId: userId,
          wordId: wordId,
        },
  ...
  const deleteAction = currentWord => {
    ...
    Alert.alert('Alert', 'Are you sure delete ' + currentWord.word + ' ?', [
      {
        text: 'Cancel',
      ...
      },
      {
        ...
        onPress: () => {
          ...
          if (objectIndex === -1) {
            ...
            axios
              .post(
                'https://nameless-spire-67072.herokuapp.com/language/deleteWord',
                {
                  id: currentWord._id,
                },
                {
        ...
        <View
          ...
          <View
            ...
            <View style={{flexDirection: 'row'}}>
              <Text style={[styles.word1, {color: colors.header}]}>
                {index + 1}/
              </Text>
              {isWord ? (
                <Text style={[styles.word, {color: colors.header}]}>
                  {item.word}
                </Text>
              ) : null}
              {isHira ? (
                ...
                {item.likes !== undefined ? (
                  <Icon
                    style={[
                      ...
                      {
                        color:
                          item.likes.length === 0 ? '#999999' : colors.header,
                      },
                    ]}
                    name="star-outline"
                    size={25}
          ...
          <View style={{flexDirection: 'row', justifyContent: 'space-between'}}>
            <View style={{flexDirection: 'row'}}>
              {isKanji ? (
                <Text style={{marginLeft: 5, textTransform: 'uppercase'}}>
                  [{item.amhan == '' ? '' : item.amhan}]
                </Text>
              ) : null}
              {isMean ? (
            ...
            {users.role !== 1 ? (
              <TouchableOpacity
                onPress={() => setMemerize(users._id, item._id)}>
                <Icon
                  style={[
                    styles.check,
                    {color: item.memerizes.length === 0 ? '#999999' : 'green'},
                  ]}
