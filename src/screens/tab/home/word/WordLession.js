...
export default WordLession = ({navigation}) => {
  ...
  useEffect(() => {
    ...
    const unsubscribe = navigation.addListener('focus', () => {
      setSearching(false);
      let arr3 = wordList.map((item, i) =>
        Object.assign({}, item, wordlevel[i]),
      );
          ...
          <View>
            {index === 0 ? (
              <Text style={{color: colors.text}}>
                {Math.floor(
                  (wordlevel.filter(e => e.memerizes.length === 1).length /
                    wordlevel.length) *
                    100,
                )}
                %
              ...
              <Text style={{color: colors.text}}>
                {Math.floor(
                  (wordlevel.filter(
                    e => e.memerizes.length === 1 && e.lession === index,
                  ).length /
                    wordlevel.filter(e => e.lession === index).length) *
                    100,
                )}
                %
              </Text>
  ...
  return (
    <View style={styles.container}>
      ...
      {searching && (
        <ScrollView
          style={[styles.itemSearch, {backgroundColor: colors.background}]}>
          {filtered.length !== 0 ? (
